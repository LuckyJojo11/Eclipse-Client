...
def run_game(username="NAME", uuid="UUID", access_token="NONE", version="26.1", modded=["Eclipse", "0"], instance_directory="", ram=4096, demo=False, instance_name=""):
    if instance_name == "":
...
def _run_game_fabric(username, uuid, access_token, version, loader, instance_directory, ram, demo, instance_name):
    ...
    # Fabric installieren, falls nötig
    if not minecraft_launcher_lib.fabric.is_minecraft_version_supported(version):
        raise RuntimeError(f"Fabric unterstützt {version} nicht")
    ...
    # Installierte Versionen durchsuchen und die richtige Fabric-ID finden
    installed_versions = minecraft_launcher_lib.utils.get_installed_versions(minecraft_directory)

    fabric_version_id = None
    for v in installed_versions:
        vid = v.get("id", "")
        if vid == f"fabric-loader-{loader_version}-{version}":
            fabric_version_id = vid
            break

    if fabric_version_id is None:
        raise RuntimeError("Fabric 0.18.4 wurde nicht gefunden")
